...
> Auto-generated from `NodeRegistry` contracts"""
    ...
    def generate_state_slices_section(self) -> str:
        ...
        for slice_name in all_slices:
            ...
            readers_str = ", ".join(f"`{r}`" for r in readers[:3])
            if len(readers) > 3:
                readers_str += f" +{len(readers)-3}"
            writers_str = ", ".join(f"`{w}`" for w in writers[:3])
            if len(writers) > 3:
        ...
        # Build relationships from data flow
        relationships = self._build_slice_relationships()
        for src, dst, label in relationships:
            lines.append(f'    {src.upper()} ||--o{{ {dst.upper()} : "{label}"')
    ...
    def _get_node_icon(self, contract: "NodeContract | None") -> str:
        ...
        if contract.requires_llm:
            return "🤖"
        if "search" in contract.name.lower():
            return "🔍"
        if "interview" in contract.name.lower():
            return "💬"
        if "like" in contract.name.lower() or "heart" in contract.name.lower():
            return "❤️"
        if "card" in contract.name.lower():
            return "🃏"
        return "📦"
    ...
    def generate_nodes_reference(self) -> str:
        ...
        for name in sorted(nodes):
            ...
            reads = ", ".join(f"`{r}`" for r in contract.reads[:3]) or "-"
            writes = ", ".join(f"`{w}`" for w in contract.writes[:3]) or "-"
